...
foreach ($stock as $tmp) {
	...
	$csv_path="./hist_data/$tmp-hist.csv";
	...
	while ($data = fgets($infile, 4096)) {
		...
		// echo $dataArr[0]."\n";
		$sql = "INSERT INTO $tmp (His_Date, Open_Price, High_Price, Low_Price, Close_Price, Volume)
			SELECT '$dataArr[0]','$dataArr[1]','$dataArr[2]','$dataArr[3]','$dataArr[4]','$dataArr[6]'
			FROM DUAL
			WHERE NOT EXISTS(
				SELECT *
				FROM $tmp
